...
START_LEARN = 500       # number of samples before learning
...
class Multi_Agent():
    ...
    def step(self, i_episode, state, action, reward, next_state, done):
        ...
        if len(self.memory) > BATCH_SIZE and i_episode > START_LEARN and i_episode % LEARN_EVERY == 0:
            for l_cnt in range(LEARN_NUM):
                for agent in self.agents:
    ...
    def learn(self, experiences, agent, GAMMA):
        ...
        for agent_idx, agent_i in enumerate(self.agents):
            if agent == agent_i:
                agent_id = agent_idx
            agent_i_current_state = next_states[:,agent_idx]
